 ...
 *
 * Notes:
 * - Images: we use inline SVG fallbacks so the page looks good even before you add real assets.
 *   Later, drop logo/banner files into /public and pass their URLs via props to <FynstraSite />.
 * - Calendly: replace CALENDLY_URL with your real link and unhide the iframe.
 */
...
// Inline SVG fallbacks so the app renders cleanly without external assets
...
const CALENDLY_URL = "https://calendly.com/"; // TODO: replace with real link
...
export default function FynstraSite({
  ...
  useEffect(() => {
    ...
    // Tiny runtime “tests” / sanity checks
  ...
  return (
    <div ref={containerRef} className="text-slate-100 bg-white selection:bg-indigo-200/60">
      {/* Global styles for minimal custom animations + brand tokens */}
      <style>{`
        :root {
        ...
        .heading-gradient { background: linear-gradient(90deg, var(--fynstra-blue) 0%, var(--fynstra-lavender) 50%, var(--fynstra-purple) 100%); -webkit-background-clip: text; background-clip: text; color: transparent; }
